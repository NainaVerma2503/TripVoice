...
IMPORTANT RULES:
1. For cities, use the exact names from: Goa, Delhi, Mumbai, Bangalore, Chennai, Kolkata, Jaipur, Hyderabad, Pune, Dubai
2. For airport codes: DEL (Delhi), BOM (Mumbai), BLR (Bangalore), MAA (Chennai), CCU (Kolkata), JAI (Jaipur), HYD (Hyderabad), PNQ (Pune), GOI (Goa), DXB (Dubai)
3. For city IDs: 700193 (Delhi), 33719 (Mumbai), 32550 (Bangalore), 33070 (Chennai), 34600 (Kolkata), 33968 (Jaipur), 33897 (Hyderabad), 35943 (Pune), 1138 (Goa), 100074 (Dubai)
4. For states: Delhi (Delhi), Maharashtra (Mumbai), Karnataka (Bangalore), Tamil Nadu (Chennai), West Bengal (Kolkata), Rajasthan (Jaipur), Telangana (Hyderabad), Maharashtra (Pune), Goa (Goa), Dubai (Dubai)
5. For dates, extract the actual date mentioned and format as DD/MM/YYYY
6. For adults, extract the number mentioned or default to 2
7. Return ONLY valid JSON, no additional text
8. If a field is not mentioned, use empty string (not null)
9. ALWAYS include cityId, city, state, country for the destination city
10. ALWAYS include from, to, depart_date, adults, intl for flight
11. ALWAYS include checkInDate and checkOutDate (same as depart_date and day after)"""
...
def plan_trip():
    """Main endpoint to plan a trip based on natural language input"""
    try:
        ...
        if previous_trip_data:
            ...
            # First time request - use the original working rule-based extraction
            trip_data, missing_fields = extract_trip_info_from_text(text)
...
@trip_bp.route('/api/trip/plan-ai', methods=['POST'])
